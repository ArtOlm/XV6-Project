...
/*
this test is used to demonstrate multiple consumers and multiple producers
*/
#define BSIZE 10
#define MAX 20
int status = 0;
...
int main(int argc, char *argv[])
{
  ...
  if(sem_post(&buffer->occupied)){
    printf("occupied semaphore was destroyed\n");
  }
  if(sem_post(&buffer->free)){
    printf("free semaphore was destroyed\n");
  }
  if(sem_post(&buffer->lock)){
    printf("lock semaphore was destroyed\n");
  }
  printf("re-initializing semaphores\n");
  sem_init(&buffer->occupied, 0);
  sem_init(&buffer->free, BSIZE);
  sem_init(&buffer->lock, 1);
